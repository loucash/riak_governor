...
-define(APP_STUB, "riak_governor").

get_deps() ->
    lists:flatten(io_lib:format("~s/dev1/lib", [relpath(current)])).

riakcmd(Path, N, Cmd) ->
    ExecName = rt_config:get(exec_name, ?APP_STUB),
    io_lib:format("~s/dev~b/bin/~s ~s", [Path, N, ExecName, Cmd]).

riakreplcmd(_Path, _N, _Cmd) ->
...
upgrade(Node, NewVersion, Config) ->
    ...
    Commands = [
        io_lib:format("cp -p -P -R \"~s/dev~b/data\" \"~s/dev/dev~b\"",
                       [OldPath, N, NewPath, N]),
        io_lib:format("rm -rf ~s/dev~b/data/*",
                       [OldPath, N]),
        io_lib:format("cp -p -P -R \"~s/dev~b/etc\" \"~s/dev/dev~b\"",
                       [OldPath, N, NewPath, N])
    ],
    [ begin
